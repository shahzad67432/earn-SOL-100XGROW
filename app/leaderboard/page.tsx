...
const Page = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-[80vh] px-4 lg:px-20 my-8 gap-8 lg:gap-16 w-full">
      {/* Left Side: Leaderboard and Tests */}
      <div className="flex flex-col w-full lg:w-2/3">
        <h2 className="text-2xl lg:text-3xl font-bold text-green-600 mb-4">Leaderboard</h2>

        {/* Scrollable Tests Container for Horizontal Scrolling */}
        <div className="flex p-2 lg:p-4 bg-gray-50 rounded-lg shadow-md gap-2 overflow-x-auto whitespace-nowrap scrollbar-thin scrollbar-thumb-green-500 mb-4">
          {tests.map((test) => (
      ...
      <div className="w-full lg:w-1/3 mt-8 lg:mt-0">
        <div className="bg-white p-4 rounded-lg shadow-md max-h-80 overflow-y-auto scrollbar-thin scrollbar-thumb-green-500">
          <h3 className="text-xl lg:text-2xl font-bold mb-4 text-green-600">Top SOL Winners</h3>
          {topUsers.length > 0 ? (
            <ul className="space-y-4">
              {topUsers.map((entry: any, idx: number) => (
                <li key={entry.userId} className="flex items-center bg-gray-100 p-3 rounded-lg shadow-sm">
                  <FaTrophy className="text-yellow-500 text-xl lg:text-2xl mr-3" />
                  <div>
                    <p className="text-base lg:text-lg font-bold truncate">{entry.user.name}</p>
                    <p className="text-xs lg:text-sm text-gray-600">Highest Score: {entry.highestScore}</p>
                  </div>
                </li>
              ))}
            </ul>
...
const Button: React.FC<ButtonProps> = ({ onClick, testId }) => {
  return (
    <button
      onClick={onClick}
      className="m-1 p-2 bg-green-500 hover:bg-green-600 text-white rounded-lg shadow-md text-xs lg:text-sm transition-all"
    >
